...
namespace cpp_robotics {
    ...
    NodeListType RRTStarDubins::planning(bool animation) {
        ...
        if (lastIndex == -1) {
            std::cout << "get best last index fail." << std::endl;
            return NodeListType();
        }
    ...
    NodeListType RRTStarDubins::genFinalCourse(int goalind){
        NodeListType path;
        path.push_back(end_);
    ...
    void RRTStarDubins::chooseParent(const vector<int>& nearinds,
                                         Node& new_node) {
        if (nearinds.empty()) {
            return;
        }
    ...
    Node RRTStarDubins::steer(const Node& rnd, int nind) {
        // expand tree
        ...
        double cost;
        dbp_.dubinsPathPlanning(nearest_node.x, nearest_node.y, nearest_node.yaw,
                                rnd.x, rnd.y, rnd.yaw,
                                curvature_,
                                px, py, pyaw,
                                mode, cost);
    ...
    int RRTStarDubins::getBestLastIndex() {
    ...
    vector<int> RRTStarDubins::findNearNodes(const Node& new_node) {
    ...
    void RRTStarDubins::rewire(const Node& new_node,
                                   vector<int> nearinds) {
        int nnode = node_list_.size();
        for (int i=0; i<nearinds.size(); i++) {
    ...
    int RRTStarDubins::getNearestListIndex(const NodeListType& node_list,
                                               const Node& rnd) {
    ...
    bool RRTStarDubins::collisionCheck(const Node& node,
    ...
    Node RRTStarDubins::getRandomPoint()
    {
        Node rnd;
    ...
    void RRTStarDubins::drawGraph(const Node& rnd) {
...
//                           1, cv::Scalar(255,255,0), -1);
    ...
    void RRTStarDubins::drawPath(const std::vector<Node>& path) {
        int cols = (max_rand_ - min_rand_) / map_resolution_;
        int rows = (max_rand_ - min_rand_) / map_resolution_;
        cv::Mat display_img(rows, cols, CV_8UC3, cv::Scalar(255,255,255));
